	...
	public void start() {
		...
		while (true) {
			String commandStr = "Please enter a Digit::  0: Store a secured attribute  "
					+ "1: Check whether a secured attribute exists  2: Remove secured attribute  "
					+ "3: List all secured attributes  4: Exit";
			...
			case 0:
				...
				char[] attributeValue = VaultInteractiveSession
						.readSensitiveValue("secured attribute value (e.g. a password)");
			...
			case 2:
				...
				try {
					...
					if (!vaultNISession.removeSecuredAttribute(vaultBlock, attributeName)) {
						System.out.println("Secured attribute "
								+ VaultSession.blockAttributeDisplayFormat(vaultBlock, attributeName)
				...
				break;
			case 3:
				System.out.println("Task:  List all secured attributes formatted as <vault-block>::<attribute>");
				System.out.println();
				try {
					List<String> attrList = vaultNISession.getSecuredAttributeList();
					if (attrList.isEmpty()) {
