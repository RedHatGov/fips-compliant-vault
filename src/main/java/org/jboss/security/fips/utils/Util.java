...
import org.jboss.security.fips.PasswordCache;
import org.jboss.security.fips.SecureExternalPasswordCache;
...
public class Util {
	 ...
	 * 
	 * @param passwordCmd
	 *            A command to execute to obtain the plaintext password. The
	 *            format is one of:
	 *            <ul>
	 *            <li>the masked password as a base64 string</li>
	 *            <li>the literal '{EXT}...' where the '...' is the exact
	 *            command line that will be passed to the Runtime.exec(String)
	 *            method to execute a platform command. The first line of the
	 *            command output is used as the password.</li>
	 *            <li>the literal '{EXTC[:expiration_in_millis]}...' where the
	 *            '...' is the exact command line that will be passed to the
	 *            Runtime.exec(String) method to execute a platform command. The
	 *            first line of the command output is used as the password. EXTC
	 *            variant will cache the passwords for expiration_in_millis
	 *            milliseconds. Default cache expiration is 0 = infinity.</li>
	 *            <li>the literal '{CMD}...' or '{CMDC}...' for a general
	 *            command to execute. The general command is a string delimited
	 *            by ',' where the first part is the actual command and further
	 *            parts represents its parameters. The comma can be backslashed
	 *            in order to keep it as the part of a parameter.</li>
	 *            <li>the literal '{CLASS[@modulename]}classname[:ctorargs]'
	 *            where the '[:ctorargs]' is an optional string delimited by the
	 *            ':' from the classname that will be passed to the classname
	 *            ctor. The ctorargs itself is a comma delimited list of
	 *            strings. The password is obtained from classname by invoking a
	 *            'char[] toCharArray()' method if found, otherwise, the 'String
	 *            toString()'</li>
	 *            </ul>
			...
			if (passwordCmdType.startsWith("EXTC") || passwordCmdType.startsWith("CMDC")) {
				...
				if (passwordCmdType.indexOf(':') > -1) {
					try {
						...
						// ignore
						FIPSLogger.LOGGER.errorParsingTimeoutNumber();
					}
				}
				if (externalPasswordCache == null) {
					externalPasswordCache = SecureExternalPasswordCache.getExternalPasswordCacheInstance();
				}
				...
				throw FIPSVaultMessages.MESSAGES.invalidPasswordCommandType(passwordCmdType);
			}
		}
		return password;
	}
	...
	private static char[] execPasswordCmd(String passwordCmd) throws Exception {
		FIPSLogger.LOGGER.traceBeginExecPasswordCmd(passwordCmd);
	...
	private static Class<?> loadClass(final String fqn, final String module) {
		try {
			Class<?> passwdClass = AccessController.doPrivileged(new PrivilegedExceptionAction<Class<?>>() {
				...
				public Class<?> run() throws Exception {
					if (fqn == null || fqn.isEmpty()) {
						throw FIPSVaultMessages.MESSAGES.loadingNullorEmptyClass();
					} else if (module == null) {
			...
			throw FIPSVaultMessages.MESSAGES.unableToLoadPasswordClass(e.getCause(), fqn);
		}
	}
	...
	/**
	 * Execute a Runtime command to load a password. It uses ProcessBuilder to
	 * execute the command.
	 * 
	 * @param passwordCmd
	...
	interface RuntimeActions {
		...
		RuntimeActions NON_PRIVILEGED = new RuntimeActions() {
			public String execCmd(final String cmd) throws Exception {
				...
				FIPSLogger.LOGGER.traceEndExecPasswordCmd(exitCode);
				return line;
			}
		};
	 ...
	 * 
	 * @param passwordCmd
	 *            a potential password command
	 * @return true if password can be loaded by {@link #loadPassword(String)},
	 *         false otherwise.
	 */
